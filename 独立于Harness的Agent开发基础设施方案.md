...
**文档版本：** 1.6  
**文档状态：** 方案设计稿  
**本版要点：** 新增配套《实施计划.md》（M0–M9、52 步），§19 增加指向实施计划的引用。  
**上一版要点：** 头部补充参考项目的本地克隆路径、HEAD 与克隆命令，§10.7 对齐基准补本地路径。  
...
- 参考实现已验证：Contrabass 以 `.contrabass/board/` 下的 JSON（issue）与 JSONL（comments）作为看板事实来源，`go.mod` 不含任何数据库依赖；RepoWiki 同样只用 JSON 状态文件（零依赖 CLI）；
- 项目级数据在数千个文件量级，扫描文本即可满足检索需求；
- 数据库的收益集中在多进程并发写与十万级记录，本设计改用文件级领取、追加写 JSONL 和 Git 合并来替代；
- 少一个依赖：换电脑、离线、CI 和代码审查都更简单，状态天然可 diff、可回滚。
...
- 可选缓存（`.cache/`）与本地文件（`local/`）不提交，任何电脑上都可以删除并重建；
...
- 任务领取写入独立租约文件，冲突时以租约时间较新者为准，并记录冲突事件；
...
- 单写者原则：同一文件同一时刻只有一个写入者；
- 原子写：写临时文件后改名替换，禁止就地截断；
- 追加型数据（事件、评论）使用 JSONL 追加（采纳 Contrabass）；
- 打开项目时对账：检测租约过期、孤儿领取、状态与产物不一致（见 §15.4）。
...
- **孤儿恢复**：启动或打开项目时，把「已领取但没有运行记录」的任务释放回 `unclaimed`，并记录恢复事件；
- **对账**：每次打开项目或执行 `devsys status` 时，重新比对工作项状态、运行记录与产物；发现不一致时记录事件，不静默修正；
...
> 详细拆分（步骤、依赖、验收、回退）见同目录《实施计划.md》；本章只给宏观阶段与阶段目标。
