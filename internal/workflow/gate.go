...
func (p *Policy) CheckGate(stage string, ev GateEvidence) GateResult {
	...
	for _, want := range gate.RequireArtifacts {
		if !names[want] {
			res.Missing = append(res.Missing, fmt.Sprintf("artifact %q is required — register one: `workloom artifact register --name %s --path <file> --actor <you> --reason <why>`", want, want))
		}
	}
	if gate.RequireMinArtifacts > 0 && len(names) < gate.RequireMinArtifacts {
		res.Missing = append(res.Missing,
			fmt.Sprintf("at least %d artifacts are required (found %d) — register one: `workloom artifact register --name <name> --path <file> --actor <you> --reason <why>`", gate.RequireMinArtifacts, len(names)))
	}
	if gate.RequireComment && ev.CommentCount < 1 {
		res.Missing = append(res.Missing, "at least one comment event is required — add one: `workloom workitem comment --id <id> --text \"...\" --actor <you>`")
	}
