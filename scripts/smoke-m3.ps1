...
function Get-WorkItemID([string]$Title) {
    $out = & $script:workloom workitem create --title $Title --actor smoke --reason smoke
...
function Step-To([string]$Id, [string]$Target) {
    & $script:workloom workitem transition --id $Id --to $Target --actor smoke --reason smoke --expect (Get-Version $Id) | Out-Null
...
try {
    ...
    & $workloom workitem transition --id $CHILD --to done --actor smoke --reason 'verified' --expect (Get-Version $CHILD) | Out-Null
    ...
    & $workloom workitem claim --id $LOWQ --owner smoke --reason 'improved' --expect (Get-Version $LOWQ) | Out-Null
    ...
    & $workloom workitem claim --id $BLOCKED_TASK --owner smoke --reason 'restored' --expect (Get-Version $BLOCKED_TASK) | Out-Null
...
"@
    ...
    & $workloom workflow start --id $FLOW_WI --policy flow --actor smoke --reason begin --expect (Get-Version $FLOW_WI) | Out-Null
    Assert-Exit 'workflow start'
    $nextOut = (& $workloom workflow next --id $FLOW_WI) -join "`n"
    ...
    & $workloom workflow step-complete --id $FLOW_WI --to implement --actor smoke --reason go --expect (Get-Version $FLOW_WI) | Out-Null
    Assert-Exit 'step-complete to implement'
    & $workloom workflow pause --id $FLOW_WI --actor smoke --reason hold --expect (Get-Version $FLOW_WI) | Out-Null
    Assert-Exit 'pause'
    ...
    & $workloom workflow resume --id $FLOW_WI --actor smoke --reason go --expect (Get-Version $FLOW_WI) | Out-Null
    Assert-Exit 'resume'
...
"@
    ...
    & $workloom workflow start --id $APPROVAL_WI --policy approval-flow --actor smoke --reason begin --expect (Get-Version $APPROVAL_WI) | Out-Null
    ...
    & $workloom workitem transition --id $APPROVAL_WI --to in_progress --actor dev --reason go --expect (Get-Version $APPROVAL_WI) | Out-Null
    Assert-Exit 'approved transition'
    $consumed = (& $workloom --json approval list --workitem $APPROVAL_WI) -join ''
    ...
    & $workloom workflow start --id $REJECT_WI --policy approval-flow --actor smoke --reason begin --expect (Get-Version $REJECT_WI) | Out-Null
    ...
    $rejected = ((& $workloom workitem get $REJECT_WI) -join "`n")
