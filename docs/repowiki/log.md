...
- 受影响 12 页（3 篇文章 + `项目接入与配置` 9 卡）全部追加「本批（#398）」注记、`source_commit` → `98c291c`；无行为变更，正文口径不变（命令分发、退出码、子命令行为均未动）。
...
- 源码基线：`ca58d27` → `19b9149`（5 提交：`32730ac` state 吸收、`cc5de1f` Git 从入场券改为可选能力、`13c4028` v0.1.8 实跑记录、`e4f1a9e` 主命令改名 `workloom` + 公开安装口径 + npm 分发层 + `setup`/`mcp install`、`19b9149` release.yml 去掉 setup-node 不接受的 `cache: false`）；任务 #397。
- 增量范围：`repowiki status --json` 报 5 提交 / 133 文件 / **42 页受影响**（3 篇文章 + 5 模块 39 卡）。知识层模块无源码变更（10 张卡保持 `source_commit: 690b294`，与本轮口径一致），仅 `知识层/架构设计.md` 因 frontmatter 缺陷被顺带修复。产物仍 55 页；`source_commit: 19b9149` 的页面从 0 → **43**。
- 手段：8 个子代理并行（5 模块批次 + 3 篇文章），沿用证据驱动外科更新口径；主代理先做一次全 bundle 机械改名（CLI 调用示例 `devsys <子命令>` → `workloom <子命令>`、`cmd/devsys|bin/devsys` 路径形态等共 **899 处**），再逐页复核并跑自建审计收口。
- 命名口径：CLI 调用示例一律 `workloom`（`cmd/workloom` 为主入口，`cmd/devsys` 为兼容别名，`scripts/install.sh` / `install.ps1` 同时装入两个名字）；刻意保持不动的 `devsys`：`.devsys/` 状态目录、`DEVSYS_*` 环境变量、`.agents/skills/devsys/`、`<!-- devsys-skill -->`、`<!-- devsys:begin/end -->`（`wire` 管理块标记原文 `<!-- devsys:begin | managed by `devsys wire` -->` 保持不动）、MCP 注册名 `devsys` 与 `mcp_servers.devsys` / `mcpServers.devsys` / `mcp.devsys`、worktree 分支前缀 `devsys/<key>`、`sync status` 的 `uncommitted-devsys` 判定码、提交前缀 `chore(devsys):`、`devsys-*`（v0.1.8 及更早的历史资产名）。
- 公开安装口径（本轮验收项）：仓库公开（MIT）；默认安装匿名下载 Release 资产、**不设 `GOPRIVATE`**；`gh auth login` / `GOPRIVATE` 只对私有 fork 有意义；`go install github.com/JAYY513/Workloom/cmd/workloom@latest` 为公开模块直装；Release 资产名自 v0.1.9 起 `workloom-*`（v0.1.8 及更早 `devsys-*`；v0.1.9 实跑 16 资产 = 6 平台二进制 + `install.sh` / `install.ps1` + `checksums.txt` + 7 个 `dist/npm/*.tgz`）；npm 层 `@jayy513/workloom` 尚未 `npm publish`（`pack-npm.sh` 只 `npm pack`，tgz 随 Release）；`workloom setup` **不注册 MCP**，`workloom mcp install` **默认只预览**（`--apply` 才写，`--force` 只替换已有 `devsys` 条目，无法安全合并即拒绝并指向 `wire --print-mcp`）。
- 计划：`.repowiki/plan.json` schema 2、6 模块 / 3 文章路径与 slug 不变；`project-access` scope 增补安装/发布面 `INSTALL.md`、`CHANGELOG.md`、`Makefile`、`.github/workflows/release.yml`、`npm/**`、`.agents/skills/devsys/**`（此前这些文件不被任何 scope 覆盖，安装/发布事实变更不会把页面标为受影响）；coverage 按新快照 317 文件重算 → **301/317**（uncovered 16 项：bundle 外用户文档、仓库配置、LICENSE、若干 *_test.go 与 templates.go）。
- 保护（D4）：起始逐页 sha256 比对 55 页，仅 `content/开发与故障诊断.md` 与生成基线不一致（人工修订，来自 `cc5de1f` 的 init 排障行，`.gitignore` 无覆盖）。**偏离记录**：规则默认「跳过」人工修改页，本轮该页同时承载失效的私有仓库口径，改为**保留人工修订行语义后重生成其余部分**（该行仍在页内，并补回 `internal/project/init.go` 行锚）；命中范围与理由记录于此，供后续轮次判断。
- 顺带修复（非本轮引入、HEAD 已存在的缺陷）：`共享应用与MCP/{概述,执行命令族,工具与Profile,错误语义,完成校验与人工复核}.md` 5 页 frontmatter **缺收尾围栏 `---`**（描述连同正文被 YAML 吞掉），`知识层/架构设计.md` 描述引号未收尾；另有 6 页 `description` 是非法 YAML（plain scalar 含 `: `）。现全部 55 页 frontmatter 均可被 `yaml.safe_load` 解析为 dict，且 8 页的 description 已按 YAML 规则加引号/转义（文本逐字保留，round-trip 断言）。
- 文档校验：`repowiki validate` = **55 files / 0 errors / 0 warnings**；主代理自建审计（`file://` 引注路径与行号、bundle 内相对链接可达、代码围栏配平、frontmatter 五键与 YAML 合法性、连续重复行、CRLF、`workloom <子命令>` 真实性）= **2082 条 `file://` 引注 0 越界 / 0 缺失，324 条 bundle 相对链接 0 死链，0 围栏失衡，0 CRLF**。
- 运行核对（黑盒，非文档推断）：在临时目录用 `bin/workloom.exe` 实跑 `workloom setup`（非 Git 目录）→ 输出 `init / workflow / wire / config / wire-check / mcp / prime / blueprint / doctor` 九行，`mcp` 行为「无项目级 MCP 配置，需要时用 `wire --print-mcp`」且**未写任何客户端配置**，`blueprint` 行为「未声明，先问目标」；`workloom mcp install`（不带 `--apply`）对三个客户端输出 `dry-run: would …`，未落盘（与文档口径一致）。
- 收尾：`repowiki state --update` 把源码基线推到本次刷新提交（自指提交，`repowiki status` 因此会显示「1 new commits, 0 files changed」——该提交只改 `.repowiki/`，不含源码位移）；本轮只改 `docs/repowiki/**` 与 `.repowiki/**`，未跑 Go 测试（无产品代码改动）。
- 副产品：复核 `workloom --help` 时发现顶层命令表缺 `init` 与 `sync status` 两行（两者仍可路由）→ 另立任务 #398。
...
- 同轮源码文案修复（任务 #361 线）：生成技能参考的 MCP core 档计数 19→20 且排除清单补 `run_cancel`（`internal/app/skill.go:61`，仓库内技能副本随 `workloom wire --skill` 重生成）；`workitem transition` 租约 fencing 报错 token 出路改本机侧车 `local/leases/<id>.token`（`internal/workitem/state.go:127`）——相关 wiki 引述已同步。
...
- 一致性发现（新）：实测 `workloom mcp serve`（默认 profile+tier core）`tools/list` 返回 **21** 项，而 `allTools()` 标定 `TierCore` 仅 19 项——`run_fail`/`run_cancel`（tier `""`）与 `run_complete`（TierCore）共用 `registerRunFinish`，tier 门按 spec 判定而注册函数一次注册三名，故二者随 `run_complete` 进入 core 档。wiki 记录偏离（`共享应用与MCP/工具与Profile.md`），产品决策另立任务 #327。
...
- 源码基线：`463c2d8`（M7.1：只读聚合层 `internal/view` + `workloom workspace view` + 触发器匹配抽到知识层；相对上次 wiki 基线 `75998a8` 2 提交、16 文件受影响）；增量模式运行。
...
- 核心内容新增：执行层五子包（harness / workspace / dispatch / retry / prompt）落到 internal/ 下，五子包之间只允许 `harness.shell` 被 `workspace.RunHook` 复用，其它互不依赖；`internal/app` 新增 Service.WorkspacePrepare/Remove/List + Dispatch（recover → retrySweep → ResolveCaps → dispatch.Plan → claim → spawn）+ RunExec（before_run + prompt.Assemble + spawn + 行级 stream + 终止守卫 + 阶段事件）+ RunPrompt（首轮/续轮 + writePromptFile）+ RunVerify（claim_head vs current_head）+ RunFinish（succeeded 走 verify 守卫 + `--force --by reviewer` 绕过）；`internal/harness` 暴露 §9.2 Adapter/Session 接口 + §9.3 CLI 形态（Shell + Codex/OpenCode/ClaudeCode）+ proc_unix/proc_windows 平台 split；`internal/workspace` 提供 Key（净化 + sha256[:16] hex 后缀）+ Root/Validate（根内不变量 + symlink 逃逸）+ Ensure/Remove/List（git worktree 生命周期）+ RunHook + HookEnv；`internal/dispatch.Plan` 是纯函数（priority desc → created_at asc → id asc + 状态白名单 + blocked_by + 全局/状态并发）；`internal/retry.Delay` 是 base/max/jitter 的可复现退避（seed=工作项 ID）；`internal/prompt.Assemble` 首轮全 brief、续轮 delta + remaining，sectioned 渲染 + sha256(text) Hash；`.devsys/runs/<id>.jsonl` 行级 sync（syncEvery=128 + 控制记录 + 末次）+ 断尾重建 + 阶段事件 run_exec_started/run_finished/record_prompt/review_routed/review_accepted；MCP `tools_run` 增 run_prompt/run_verify（session profile）+ run_complete/run_fail/run_cancel（executor profile）；`config.yaml` 增可选 `workspace_root`（默认 `<project>/.devsys/workspaces`）+ `dispatch_command`（默认 `workloom run exec --id {run_id} --actor dispatch --reason tick`）；`DEVSYS_PROJECT_ROOT` 让 agent 在 git worktree 内运行时把报告写回主仓库的 `.devsys/`。
