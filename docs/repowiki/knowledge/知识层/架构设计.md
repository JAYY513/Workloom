...
internal/app/                    # M4 共享应用服务
   ...
   ├── context.go                # M5：knowledgeContext + KnowledgeContext + KnowledgePageRef + triggerHit + MatchTrigger/MatchSources
...
internal/mcp/                    # 官方 Go SDK；M5 增 knowledge_validate / knowledge_refresh / context_for_workitem paths
   ...
   ↓
M5 新增主模块：
internal/knowledge/              # 知识层业务规则（八子文件 + 一锁复用）
...
**匹配规则**：
- `MatchTrigger`：触发词在 lowercased 文本中作为子串出现；多个命中保留第一个（按 page 遍历顺序）；同一页面已记录就不再覆盖。
- `MatchSources`：sources 列表中任意 pattern 与 paths 中任意路径命中（按 `knowledge.Match` 语法）；`sources` 为空时回退到 `state.Mapping(path).Sources`。
...
| `state.go` | `storage.AtomicWrite` | 全部 | state 加载与映射 |
