...
`KnowledgeStatus`（[internal/app/knowledge.go:64-148](../知识层/架构设计.md)）按以下顺序决定 `KnowledgeStatusView.Status`：
...
`KnowledgeStatusView.Reason`（[internal/app/knowledge.go:96-108](../知识层/架构设计.md)）按上面四类拼一句话：
