...
1. 收集风险：过期租约 → 孤儿领取 → 不可读调度文件 → 滞留审查（`review`/`verification` 且 `UpdatedAt ≤ now − ReviewStaleAfter(24h)`）→ 阻塞 → 质量门被拒（`quality_blocked`，b89ffae：ready 工作项经 `next.QualityBlocks` 与 `app.policyForWorkItem` 同源评分命中 `policy.QualityGateBlocks`）→ 重试等待（`retry_pending`，b89ffae：`retry_queued` 且 `NextAttemptAt` 记录在未来 / 已到期）→ 元数据非法 → 策略非法（含「工作项引用缺失策略」）→ 待决审批 → **空项目**（`empty_project`，v0.1.3 #337：无工作项时给出 `CreateWorkitemCommand` 补救，未声明蓝图再追加 `devsys project update --blueprint-artifact` 提示）→ **dead_attempt / in_flight**（v0.1.3 #336：`ClassifyAttempts` 把 in-progress claim 按最新 run 证据分为死尝试与健康在途；dead_attempt 占 §7.4 rung 1 推荐先 recover，in_flight 只是 CONCERNS 信号，推荐仍可落在 `report_done`）。
2. 判定：待恢复事务数 > 0 → **FAIL**，并把 `devsys recover …` 写入 `Fixes`；否则有任一风险 → **CONCERNS**；无 → **PASS**。
3. 推荐：`recover_claim`（过期/孤儿，以及 dead_attempt）→ `review`（review/verification 或有 pending 审批，按 `updated_at` 升序）→ `start`（ready，按 §7.4 dispatch 排序：priority 降序 → created_at 升序 → id；若推荐目标命中 `QualityBlocks`，`Reason` 写入 `score X below min_score Y + improvements`，仍指回该任务但提醒先用 `devsys workitem update --id …` 改稿）→ `start_backlog`（backlog 按 created_at 升序；项目声明了 `PolicyIDs` 但未设 `DefaultPolicy` 且推荐项无 `Workflow` 实例时，`Reason` 追加 `; no workflow policy is bound to <id> and config.yaml declares no default_policy, so quality and stage gates are not enforced`）→ `milestone_review`（首个未完成 milestone）→ `report_done`（若有 `retry_queue` 且无 runnable 任务，`Reason` 追加 `; N work item(s) wait for a dispatch retry (...) — run \`devsys dispatch --once\``；若有 in_flight attempt，再追加 `; N attempt(s) still in flight (...) — wait or inspect \`devsys run get --id <run-id>\``）。
...
- **#342 质量门中日韩加权与门禁补救命令**：`workflow.ScoreQuality` 长度分量用 `weightedLen`（中日韩表意文字双倍宽度），验收语言分量由结构化 `acceptance_criteria` 直接满足；`Policy.CheckGate` 的 missing 文案附补救命令（`devsys artifact register` / `devsys workitem comment`），判定与退出码不变（[internal/workflow/quality.go:29-46](file://internal/workflow/quality.go#L29-L46)、[internal/workflow/quality.go:77-98](file://internal/workflow/quality.go#L77-L98)、[internal/workflow/gate.go:59-77](file://internal/workflow/gate.go#L59-L77)）。
