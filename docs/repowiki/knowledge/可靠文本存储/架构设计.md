...
triggers:
  ...
  - 守卫更新
  - 版本链分叉
description: storage 三层基元之上 M1 五个领域模块、M2 调度 / reconcile 与 M3 工作流 / 审批 / next 三层、M4 显式化 ReadSnapshot + 守卫 Update（record/update.go、run/ReadSnapshot），artifact 版本链线性、防分叉（ErrSuperseded）；M5 新增 `internal/storage.LockFile`（39 行，OS 独占锁原语，被知识层 `LockRefresh` 复用）与方案 §15.2 同源（`ErrLocked` + `O_CREATE|O_RDWR` + `lockHandle` + 释放 = unlock + close）
generated: true
