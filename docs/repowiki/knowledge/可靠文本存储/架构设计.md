...
| `next`（M3，b89ffae 扩） | （无落盘） | `Evaluate(in Input) Report` + `QualityBlocks(items, source PolicySource)`（与 claim 同源评分） | 纯函数；§7.4 固定六优先级推荐；FAIL = 有待恢复事务（§15.4 不输出业务事实）；CONCERNS = 任一风险信号（含 `quality_blocked`、`retry_pending`，b89ffae）；PASS = 无；exit 恒 0（评估成功），便于脚本按 `verdict` 分支；`Input` 增 `QualityBlocks / DefaultPolicy / PolicyIDs`（`app.policyForWorkItem` 唯一解析点） | [internal/next/evaluate.go:1-470](file://internal/next/evaluate.go#L1-L470) |
...
| `reconcile`（M8.2 增量） | `reconcile.go:45-49` / `reconcile.go:306-364` / `reconcile.go:501-509` | `ProposalNoteUnmerged` 枚举 + `conflictNotes` 探针 + `RepairApply` 对该 Kind 的 rejected 分支 | 复用知识层 `PorcelainStatus` / `MergeHeads`；git 失败降级 plan note（`RepairDryRun` 不报错）；apply 永远全 rejected，零写盘、不走 Guard、不走 `ApplyRepair`；`ComputeDigest` 的第三键 `firstEvidence.Path` 仅 disambiguate 当前单 evidence kind | [internal/reconcile/reconcile.go:45-49](file://internal/reconcile/reconcile.go#L45-L49)、[internal/reconcile/reconcile.go:306-364](file://internal/reconcile/reconcile.go#L306-L364)、[internal/reconcile/reconcile.go:501-509](file://internal/reconcile/reconcile.go#L501-L509) |
| `reconcile`（M#337 增量） | `reconcile.go:89-114` / `reconcile.go:222-231` / `reconcile.go:320-333` / `reconcile.go:889-1003` | `InvalidFile{Path, Err}` + `InspectionReport.InvalidFiles` + `Doctor` 填充 + Note 追加 + `RepairDryRun` fail-closed；`readAllWorkitems` / `buildProposals` 解码失败继续扫其余文件 | `readAllWorkitems` 走 `storage.Inspect` 解码单个失败 → `InvalidFile` + continue；`RepairDryRun` `len(invalid) > 0` → `fmt.Errorf("cannot plan repairs: %s", …)`；CLI doctor `INVALID  <path>  <err>` 行 + `--json` / 人类模式均 `exitWithCode(CodeInvalid)`；next / prime / project status 同路 `Invalidf(KindInvalid)` | [internal/reconcile/reconcile.go:89-114](file://internal/reconcile/reconcile.go#L89-L114)、[internal/reconcile/reconcile.go:222-231](file://internal/reconcile/reconcile.go#L222-L231)、[internal/reconcile/reconcile.go:320-333](file://internal/reconcile/reconcile.go#L320-L333)、[internal/reconcile/reconcile.go:889-1003](file://internal/reconcile/reconcile.go#L889-L1003) |
...
`scheduling/` 受管路径同 `workitems/`，由 `workitem.leaseRel` 派生（[internal/workitem/claim.go:946-948](file://internal/workitem/claim.go#L946-L948)）。`reconcile` 通过 `storage.Inspect` 走只读路径，never `storage.Read`（[internal/reconcile/reconcile.go:1-20](file://internal/reconcile/reconcile.go#L1-L20)）。
...
`workitem.Create` ([internal/workitem/workitem.go:55-88](file://internal/workitem/workitem.go#L55-L88))、`run.Create` ([internal/run/run.go:51-92](file://internal/run/run.go#L51-L92))、`record.create` ([internal/record/record.go:124-153](file://internal/record/record.go#L124-L153))、`approval.nextNumber` ([internal/approval/approval.go:431-450](file://internal/approval/approval.go#L431-L450)) 共享同一结构：
...
`workitem.Update` ([internal/workitem/workitem.go:156-164](file://internal/workitem/workitem.go#L156-L164))、`run.Update` ([internal/run/run.go:194-202](file://internal/run/run.go#L194-L202))、`approval.DecideTx` ([internal/approval/approval.go:210-257](file://internal/approval/approval.go#L210-L257))、`approval.ConsumeTx` ([internal/approval/approval.go:266-310](file://internal/approval/approval.go#L266-L310)) 走相同契约：
...
[internal/next/evaluate.go:180-259](file://internal/next/evaluate.go#L180-L259) 是 §7.4 落点：

1. 收集风险：过期租约 → 孤儿领取 → 不可读调度文件 → 滞留审查（`review`/`verification` 且 `UpdatedAt ≤ now − ReviewStaleAfter(24h)`）→ 阻塞 → 质量门被拒（`quality_blocked`，b89ffae：ready 工作项经 `next.QualityBlocks` 与 `app.policyForWorkItem` 同源评分命中 `policy.QualityGateBlocks`）→ 重试等待（`retry_pending`，b89ffae：`retry_queued` 且 `NextAttemptAt` 记录在未来 / 已到期）→ 元数据非法 → 策略非法（含「工作项引用缺失策略」）→ 待决审批。
2. 判定：待恢复事务数 > 0 → **FAIL**，并把 `devsys recover …` 写入 `Fixes`；否则有任一风险 → **CONCERNS**；无 → **PASS**。
3. 推荐：`recover_claim`（过期/孤儿）→ `review`（review/verification 或有 pending 审批，按 `updated_at` 升序）→ `start`（ready，按 §7.4 dispatch 排序：priority 降序 → created_at 升序 → id；若推荐目标命中 `QualityBlocks`，`Reason` 写入 `score X below min_score Y + improvements`，仍指回该任务但提醒先用 `devsys workitem update --id …` 改稿）→ `start_backlog`（backlog 按 created_at 升序；项目声明了 `PolicyIDs` 但未设 `DefaultPolicy` 且推荐项无 `Workflow` 实例时，`Reason` 追加 `; no workflow policy is bound to <id> and config.yaml declares no default_policy, so quality and stage gates are not enforced`）→ `milestone_review`（首个未完成 milestone）→ `report_done`（若有 `retry_queue` 且无 runnable 任务，`Reason` 追加 `; N work item(s) wait for a dispatch retry (...) — run \`devsys dispatch --once\``）。

CLI `devsys next` ([internal/cli/cli.go:1569-1615](file://internal/cli/cli.go#L1569-L1615)) 是只读组装入口：先调 `reconcile.Doctor` 拿待恢复事务与租约风险；仅在 `InspectionOK && len(pending) == 0` 时列举 workitems 与 pending approvals（不可信状态不输出业务事实，§15.4）；策略问题与缺失策略引用合并入 `PolicyProblems`，并把 `app.policyForWorkItem` 唯一解析出的项目级 `default_policy` 与所有声明策略 ID 一并送入 `Input.DefaultPolicy / PolicyIDs`（b89ffae）。输出 `readiness: PASS|CONCERNS|FAIL` + reasons + risks + fixes + next，exit 恒 0。
...
- **Run 读取**：`internal/run/run.go:156-185` 的 `ReadSnapshot(ctx, id) (*domain.Run, []byte, error)` 同形单态实现。
...
- [internal/run/run.go:156-185](file://internal/run/run.go#L156-L185)
...
| `next/*`（M3，b89ffae 扩） | `domain.AllStatuses`、`domain.StatusRetryQueued`、`reconcile.LeaseProbe` 类型、`storage.PendingTxn` 类型、`workflow.Policy.QualityGateBlocks`（与 claim 同源评分）、`workflow.ScoreQuality`（用于 `QualityBlocks` 辅助函数） | 纯函数 `Evaluate` + `QualityBlocks`；CLI `devsys next` 负责组装 Input（含 `QualityBlocks` / `DefaultPolicy` / `PolicyIDs`） |
...
- **b89ffae 阶段门失效审批补充**：`approval.InvalidatedFor(ctx, root, wiID, stage)` 是只读辅助；`app.checkTransitionGate` 在没有匹配 `MatchingGate` 审批时调它，把最近一次作废的 stage_gate 审批写入 `workflow.GateEvidence.ApprovalNote`，再由 `Policy.CheckGate` 追加到 `require_approval` 的 missing 文案末尾（[internal/workflow/gate.go:18-23](file://internal/workflow/gate.go#L18-L23)、[internal/workflow/gate.go:66-74](file://internal/workflow/gate.go#L66-L74)）。判定与退出码不变。
