...
triggers:
  ...
  - readAllWorkitems
  - Inspect 扫工作项
description: storage 事务协议（journal + commit marker + payload）、recover 重放与 verifyUntouched 校验；M3 新增 AppendJSONLRaw（多行 JSONL payload）与 Tx.Staged（事务内已暂存查询）；M#337（commit 362b637）reconcile 的 `readAllWorkitems` 走 `storage.Inspect` 扫工作项文件，解码失败记入 `InvalidFile` 并继续扫其余——纯只读探针，与事务 / commit / recover 协议无耦合；v0.1.4（9aab68f，#342）起租约 token 侧车（`local/leases/<id>.token`）也走同一事务协议——`stageToken` / `dropToken` 在 claim / release 事务内以 `ExpectAny` 写删侧车，提交两侧语义不变；本轮（ca58d27→19b9149，v0.1.9 发布链 + 主命令改名 workloom + Git 可选能力）：事务协议本身未改，仅 `verifyUntouched` 的 `ErrRecoveryFailed` 详情文案把 `outside devsys` 改为 `outside workloom`。
generated: true
...
## 本批（v0.1.4，commit 9aab68f #342）：token 侧车也走事务协议

#342 把租约 fencing token 从受管状态树移到本地侧车 `local/leases/<id>.token`，侧车的写删**完全复用同一事务协议**：

- `stageToken(tx, id, token)` 在 `Claim` 事务内 `tx.Put(ExpectAny)` 写侧车——`ExpectAny` 容忍中断 claim 留下的过期侧车；`dropToken(tx, id)` 在 `Release` / `QueueRetry` 事务内 `tx.Delete(ExpectAny)` 删除（[internal/workitem/lease_token.go:19-28](file://internal/workitem/lease_token.go#L19-L28)）。
- commit 前崩溃 → 侧车随事务一起丢弃（`verifyUntouched` 不校验 `local/leases/`，该目录不受 `Expect` 管辖）；commit 后崩溃 → 重放幂等，`ExpectAny` 预写不冲突。
- 受管路径白名单不变：`local/lock` 与 `local/txn` 仍被 `ErrUnsafePath` 拒绝，`local/leases/` 是 `local/` 下的合法子路径（[internal/storage/store.go:90-106](file://internal/storage/store.go#L90-L106)）。

## 本批（v0.1.9，ca58d27→19b9149）

本轮事务协议**没有**结构或语义变化；本卡只就地修正两处随主命令改名而变的错误文案：

- `verifyUntouched` 的两条拒绝文案由「target changed outside devsys while the transaction was interrupted (expected %s, found %s)」改为「target changed outside **workloom** …」——`Expect` / `PreSize` / 大小校验的判定条件与拒绝语义完全不变（[internal/storage/recover.go:165](file://internal/storage/recover.go#L165)、[internal/storage/recover.go:186](file://internal/storage/recover.go#L186)）。
- 项目级锁超时（无 holder 信息时）的文案由「%s is held by another devsys process」改为「%s is held by another **workloom** process」（[internal/storage/lock.go:54](file://internal/storage/lock.go#L54)）。
- 行锚复核：`internal/storage/{store,txn,recover,inspect}.go` 的行数在本轮未变（全部改动都是同行替换），本卡引用的 `recover.go:34-85` / `89-141` / `218-330`、`txn.go:179-199` / `241-247` / `258-343` / `354-365` / `436-441`、`store.go:143-166` / `172-190` 仍然成立。
- 未受影响的部分：journal / commit marker / payload 布局、`replayTxn` 幂等规则（hash / size 比对 + `skipped`）、`AppendJSONLRaw` 与 `Tx.Staged` 的约束、`local/leases/` 侧车的 `ExpectAny` 语义、`Inspect` / `InspectUnlocked` 的只读边界。
