...
`approval.InvalidateForStatusTx` 是其典型消费者（M3Reviewer6 ③，[internal/approval/approval.go:485-533](file://internal/approval/approval.go#L485-L533)）：失效 pass 必须跳过本事务已被 `ConsumeTx` 改写的审批文件，否则会触发 `staged twice` 断言。
