...
triggers:
  ...
  - 非 nil 空 slice
  - JSON 序列化 []
description: 工作项九状态机与保守转换、调度态与文件级领取/心跳/释放/重试、M3 新增 TransitionRequest.Guard、propagateRecordsTx 完成事务广播、workflow_instance.go 五类实例操作（只动 workflow 字段、租约 fencing）；d726ed4 起所有读写入口前移 `ValidID` 拦截（非法 id 直接 `ErrBadID`、usage/exit 2，不再落 storage 的 `unsafe managed path` 兜底），并把租约活跃时的 `ErrLeaseTokenMismatch` 详情升级为「`workloom workitem release --id <id> --owner <owner> --token <token from .devsys/local/leases/<id>.token>`」提示；M#337（commit 362b637）`workitem.Store.List` 空集合返回非 nil `[]*domain.WorkItem{}`（下游 `len(...) == 0` 判定与 JSON 序列化 `[]` 不再需要 nil 守卫）；v0.1.4（9aab68f，#342）起 fencing token 移出受管树存 `local/leases/<id>.token` 侧车（`Heartbeat`/`Release`/`workflowWrite` 经 `tokenFromTx` 校验、旧 persisted 值兜底），`ReleaseOptions` 增 `ForCompleted`/`ForRefused` run-bound 释放；本轮（ca58d27→19b9149，v0.1.9 发布链 + 主命令改名 workloom + Git 可选能力）：租约相关的三条报错 / 提示文案改指 `workloom`（`workloom recover`、`workloom workitem release …`），状态图、转换矩阵、SchedulingState 与四种释放路径语义未变。
generated: true
...
- `Release` 删除租约文件、删除侧车（`dropToken`），并把工作项调度态回到 `released`（要求 `Token/Owner` 匹配，经侧车校验；[internal/workitem/claim.go:448-563](file://internal/workitem/claim.go#L448-L563)）。缺 owner/token 时报错并指明「expired or orphaned leases are recovered with `workloom recover` instead」（[internal/workitem/claim.go:488-490](file://internal/workitem/claim.go#L488-L490)）。
- `ReleaseOptions` 的恢复路径 `ForExpired=true`（要求已验证 `lease_until < now`）与 `ForOrphan=true`（要求绑定 run 文件缺失）**绕过 owner/token**，由 `workloom recover` 使用；#342 起新增 run-bound 释放 `ForCompleted=true`（要求租约属于已 `succeeded` 的绑定 run）与 `ForRefused=true`（要求绑定 run 带有记录的拒绝 `Verification.Advanced=false`），以 run 记录为证明、不要求 owner/token，也不会误杀更新的 claim（[internal/workitem/claim.go:100-122](file://internal/workitem/claim.go#L100-L122)）。
...
| 操作 | 入口 | 副作用 | 拒绝面 |
|---|---|---|---|
| `WorkflowStart` | `workloom workflow start --id … --policy … --actor … --reason … [--expect …] [--owner … --token …]` | 写 `wi.Workflow = {ID: policy.ID, Step: policy.Steps[0].ID}` + `workflow_started` 事件；首步声明 `status` 时要求当前工作项状态匹配 | 已存在 instance、终态工作项、首步 status 不匹配 |
| `WorkflowStepComplete` | `workloom workflow step-complete --id … [--to …] --actor … --reason … [--expect …] [--owner … --token …]` | 按声明序取首满足候选（或显式 `--to`）；目标步骤声明 `status` 时要求当前状态匹配；自环拒绝 | 无 instance、instance 终态、instance paused、`--to` 未声明或条件不满足、目标 status 不匹配 |
| `WorkflowPause` / `WorkflowResume` / `WorkflowCancel` | `workloom workflow pause/resume/cancel --id … --actor … --reason … [--expect …] [--owner … --token …]` | `Workflow.Paused` 或 `Step = "cancelled"` + 对应事件 | 无 instance、终态 / 已暂停 / 未暂停 / 已取消 |
...
工作项 / 状态机 / 工作流错误在 CLI 层统一走 `app.storeError` + `app.Error{Kind}`（[internal/app/workitem.go:460-484](file://internal/app/workitem.go#L460-L484)）；各 handler 用 `errUsage / errPrecondition / errInvalid` 三个工厂（[internal/cli/cli.go:127-147](file://internal/cli/cli.go#L127-L147)）造 `codedError`：
...
```sh
# 状态机转换
V=$(bin/workloom.exe --json workitem get WLM-1 | jq -r .version)
bin/workloom.exe workitem transition --id WLM-1 --to in_progress \
    --actor alice --reason "kickoff" --expect "$V"

# 审批门禁（默认 on_reject=blocked）
bin/workloom.exe approval request --id WLM-1 --stage in_progress --actor alice --reason "gate"
bin/workloom.exe approval approve --id approval-1 --by boss --comment ok
# 转换在同一事务消费：工作项进入 in_progress + approval-1 consumed

# 工作流实例推进（要求当前 lease owner + token）
bin/workloom.exe workflow start --id WLM-1 --policy quick-fix --actor alice --reason begin --expect "$V" \
    --owner alice --token <lease-token>
bin/workloom.exe workflow step-complete --id WLM-1 --actor alice --reason next --expect "$V" \
    --owner alice --token <lease-token>
```
...
- 这是与 reconcile `readAllWorkitems`（M#337 同时段新增的不可信集合）的对比：`readAllWorkitems` 仍返回 `var items []*domain.WorkItem`（可 nil），因为它面向对账子路径、需要「无文件」与「空文件集合」区分；`Store.List` 面向 `workloom workitem list` / MCP `workitem_list` 等消费者，统一返回非 nil 让下游可直接 `len(...) == 0` 判定 / JSON 序列化 `[]` 而无需 nil 守卫。
...
主命令改名 `workloom` 触及本卡引用的三条租约/状态报错文案；状态机与租约语义**未变**：

- `Release` 在既非恢复路径又缺 `Owner`/`Token` 时报「owner and token are required (expired or orphaned leases are recovered with `workloom recover` instead)」（[internal/workitem/claim.go:489](file://internal/workitem/claim.go#L489)）。
- 离开 `in_progress` 而租约仍活跃时，`ErrLeaseTokenMismatch` 的详情给出 `workloom workitem release --id <id> --owner <owner> --token <token from .devsys/local/leases/<id>.token> --actor <actor> --reason <reason>`（[internal/workitem/state.go:127](file://internal/workitem/state.go#L127)）。
- 活跃租约下走普通 `Store.Update` 被拒时的文案是「release it first (`workloom workitem release --id <id> --owner <owner> --token <token> --actor <you> --reason <why>`) or update through the claim holder」（[internal/workitem/workitem.go:184](file://internal/workitem/workitem.go#L184)）。
- 未受影响：九态图与转换矩阵、`changeStatus` 自动写 `SchedulingState`、`SchedulingClaimed` 必须有租约文件 / `SchedulingReleased` 必须已释放、`ForExpired` / `ForOrphan` / `ForCompleted` / `ForRefused` 四种绕过 owner/token 的释放路径、`workflow_instance.go` 只动 `workflow` 字段。
- 行锚复核：`internal/workitem/{claim,state,workitem}.go` 行数本轮未变（改动都是同行替换），本卡引用的 `claim.go:448-563` / `100-122` / `1031-1037`、`state.go:143-149` / `159-161` / `162-175`、`workitem.go:131-164` / `133-171` / `173-227` 仍然成立。
