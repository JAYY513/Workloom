...
- **本批（v0.1.11 / npm 首发）**：v0.1.11 发布（npm 首发 + 安装口径收敛 + npm 平台包拒绝文案，[CHANGELOG.md:17-28](file://CHANGELOG.md#L17-L28)）；执行层接线与 `DEVSYS_PROJECT_ROOT` 注入不变，本页口径不变，`source_commit` 跟进至 `238e30c`。
