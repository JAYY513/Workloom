...
| `dispatch --once\|--watch\|--dry-run\|--max` | — | admin | `Service.Dispatch(ctx, DispatchRequest{...})` | `internal/dispatch.Plan` + `internal/workspace.Ensure` + `internal/app.dispatchOne`（包含 `internal/prompt.Assemble`）；**v0.1.13 起** attempt 的启动命令由 `internal/app.attemptSpawnCommand` 解析（与 MCP 注册同一 `ResolveMCPCommand`，npm 树内 `node <wrapper>/bin/workloom.js`，[internal/app/dispatch.go:440-452](file://internal/app/dispatch.go#L440-L452)） |
...
- `workloom mcp serve` 0 工具分支（[internal/cli/mcp.go:188-201](file://internal/cli/mcp.go#L188-L201)）：构造 cfg 后调 `mcp.VisibleTools(cfg)`，拿到空 slice 走 `errUsage("mcp serve: %s %s %s no tools in tier %s; use --tier standard", word, strings.Join(quoted, ", "), verb, tier)`（单 profile 渲染为 `mcp serve: profile "session" has no tools in tier core; use --tier standard`） → `CodeUsage = 2`。多 profile 时单复数与动词变 `profiles %s have`。避免 silent server 的隐藏陷阱。
- `app.MCPSnippet` 三参签名（[internal/app/mcpsnippets.go:21](file://internal/app/mcpsnippets.go#L21)）：`MCPSnippet(name, devsysBin, projectRoot)`；`bin` 与 `cwd` 都 `filepath.ToSlash(strings.TrimSpace(…))`；`jsonString` 走 `encoding/json` 编字符串。`internal/cli/cli.go` 旧 `MCPSnippet(name, os.Args[0])` 替换为 `(name, os.Args[0], svc.Root)`。
...
- **本批（v0.1.11 / npm 首发）**：v0.1.11 发布（npm 首发 + 安装口径收敛 + npm 平台包拒绝文案，[CHANGELOG.md:17-28](file://CHANGELOG.md#L28-L39)）；执行层接线与 `DEVSYS_PROJECT_ROOT` 注入不变，本页口径不变，`source_commit` 跟进至 `238e30c`。
- **本批（v0.1.12 / MCP 接入闭环）**：执行层接线与 `DEVSYS_PROJECT_ROOT` / `HookEnv` 注入**不变**；本页受影响的是同一族「用哪个本程序」的解析——`mcp install` 与 `wire --print-mcp` 改走 `app.MCPCommandFor(os.Args[0])`（npm 安装树内写 `node` + `<wrapper>/bin/workloom.js`，[internal/app/mcpcommand.go:53-74](file://internal/app/mcpcommand.go#L53-L74)），`mcp install` 写入后现场探一次服务（`--apply` 失败 exit 3、配置保留）。**已知限制（记录在案、未修）**：`internal/app/dispatch.go` 的 `defaultSpawner` 仍用 `os.Executable()`（[internal/app/dispatch.go:403-410](file://internal/app/dispatch.go#L403-L410)），npm 装机用户经 `dispatch`/`run exec` 起的 attempt 会拿到 `node_modules` 里的嵌套路径（属 M6 语义，留给 npm 渠道全量巡检）；pnpm 的 store 路径可被 spawn，但 store 修剪后失效（npm 是文档唯一承诺的安装方式）。`source_commit` 跟进至 `7cdd918`。
- **本批（v0.1.13 / #414+#416）**：执行层接线与 `DEVSYS_PROJECT_ROOT` / `HookEnv` 注入**不变**；本批修掉 v0.1.12 记录的已知限制——`defaultSpawner` 不再直接 `exec.Command(os.Executable(), argv…)`，改由 `attemptSpawnCommand`（[internal/app/dispatch.go:440-452](file://internal/app/dispatch.go#L440-L452)）把 exe 交给 `ResolveMCPCommand` 解析后拼接 devsys 子命令 argv：npm 安装树内 attempt 经 `node <wrapper>/bin/workloom.js` 启动（与 MCP 注册同一解析），Release / `go install` / 源码构建仍原样 spawn 二进制（[internal/app/dispatch.go:403-414](file://internal/app/dispatch.go#L403-L414)）。`dispatchCommandLine` / `dispatchOne` 映射与 `run exec` 接线不变，新增 `internal/app/dispatch_spawn_test.go` 固定两类形态；发布侧改 npm OIDC trusted publishing（非本页接线）。
